...
class UsuariosController
{
    ...
    public function criar(): void
    {
        ...
        if (!in_array($perfil, ['admin', 'aluno', 'atendente'], true)) {
            http_response_code(400);
            echo json_encode(['erro' => 'Perfil invalido.']);
            return;
        }
        ...
        try {
            ...
            echo json_encode(['erro' => 'Erro ao cadastrar usuario. E-mail pode estar duplicado.']);
        }
    }
    ...
    public function atualizar(): void
    {
        ...
        if (!$id || $nome === '' || $email === '') {
            http_response_code(400);
            echo json_encode(['erro' => 'ID, nome e e-mail sao obrigatorios.']);
            return;
        }
        ...
        try {
            // Se enviou senha, atualiza também
            if ($senha !== '') {
                $senhaHash = password_hash($senha, PASSWORD_DEFAULT);
                $sql = 'UPDATE usuarios
                        SET nome   = :nome,
